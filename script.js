...
{question:'How many times can Vivo eat in one day?',answers:['2 times','3 times','5 times','Every time he sees food'],correct:3,yes:'Correct! Vivo has 10000000% interest on foods. 🍜🏆',no:'Oops, incorrect! Vivo’s stomach capacity is clearly being underestimated. Count those meals again! 🍽️😭'},
{question:'Why is Vivo never full?',answers:['He has a second stomach','The food serve is too small','He has the appetite of a hungry worm','Jenni hasn’t fed him enough'],correct:2,yes:'Correct! The birthday boy is powered by an unlimited food supply. 🪱😭',no:'Not quite! The mystery of Vivo’s endless appetite remains unsolved. Try again, hungry boy! 🪱😂'},
...
nb.onclick=()=>{qi++;if(qi<questions.length){renderQuestion();return}showScreen(4);document.getElementById('finalScore').textContent=`${score}/${questions.length}`;const msgs=['Your stomach understands Jenni better than your brain does. 😭🍜','Your stomach understands Jenni better than your brain does. 😭🍜','Your girlfriend knowledge needs a little seasoning. Maybe ask for a snack break? 🍪🧠','Three correct answers! Basically the same number of meals you eats before noon. 🍽️','Not bad! One answer probably got distracted by the thought of food. 🍜😭','PERFECT SCORE! 🏆❤️ Jenni is officially impressed. 😌✨'];document.getElementById('scoreMessage').textContent=msgs[score]};
document.getElementById('acceptMission').onclick=()=>showScreen(8);
document.getElementById('laterMission').onclick=()=>{const f=document.getElementById('finalFeedback');f.textContent='ARE YOU SURE? 🚨 The final boss is disappointed.👿 Also, your girlfriend has already planned the date, so… nice try. 😭💅';f.classList.remove('hidden')};
